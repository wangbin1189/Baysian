function [Model]    = LRM_Panel_GibbsSampling3(Model)
...
for s = 1:Model.S
    ...
    for n = 1:Model.N
        ...
        e2          =   e(find(~isnan(e)));
        Model.S2_Errors_Ps(n) = e2'*e2;                     %Sum of squared errors due to individual
        Model.S2_Error_Ps    = Model.S2_Error_Ps + e2'*e2;  %Add sum of squared errors due to individual
        
        Model.S2_Errors_Ps(n) = (Model.S2_Errors_Ps(n) + Model.DS2_Errors_Pr(n))/Model.D_Errors_Ps(n);                     
        Model.H_Errors_Drw(n) = gamm_rnd(1,1, .5*Model.D_Errors_Ps(n), .5*Model.D_Errors_Ps(n)*Model.S2_Errors_Ps(n));                
    end
    ...
    tmp1                = Model.V_Theta_Inv * mean(Model.ThetaDraw')';              %Eq 7.27 Line 3
    tmp2                = Model.V_MTheta_Inv_Pr * Model.M_MTheta_Pr;                %Eq 7.27 Line 3
    Model.M_MTheta_Ps   = Model.V_MTheta_Ps*(tmp1 + tmp2);
    Model.MTheta        = mvnrnd(Model.M_MTheta_Ps,Model.V_MTheta_Ps,1);            %Eq 7.26
    % DRAW THETA
