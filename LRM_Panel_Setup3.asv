...
Model.GoodObs           =   sum(isnan([Model.X Model.Y])')'==0; %1 if all data present in a row
...
Model.H2_Error_Pr   =   0;
Model.S2_Error_Pr   =   0;
Model.S2_Errors_Ps  =   zeros(Model.N,1); %For heteroscedastic errors
...
Model.V_MTheta_Pr       =   5*eye(Model.K);     %Allow substantial variation in prior
...
Model.D_Error_Pr    =   1;                  %Prior degrees of freedom eq 7.23
Model.H2_Error_Pr   =   1/Model.SE_OLS;     %starting value of h, the standard errors from the OLS regression
Model.S2_Error_Pr   =   1/Model.H2_Error_Pr;

Model.S2_Errors_Pr  =   1/Model.H2_Error_Pr;%Hetero errors
